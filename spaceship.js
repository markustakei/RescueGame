...
function RenderSpaceship(context) {
  ...
  var rotatedPoint = RotateAroundOrigin(
    POLICE_MAN.positions[0].x,
    POLICE_MAN.positions[0].y,
    POLICE_MAN.rotation
  );
  context.moveTo(POLICE_MAN.x + rotatedPoint[0],POLICE_MAN.y +  rotatedPoint[1]);
  POLICE_MAN.latest.x = POLICE_MAN.x + rotatedPoint[0];
  POLICE_MAN.latest.y = POLICE_MAN.y + rotatedPoint[1];
  // Begin rendering the space ship points (rotating them each time)
  context.beginPath();
  for (var i = 0; i < POLICE_MAN.positions.length; i++) {
    var rotatedPoint = RotateAroundOrigin(
      POLICE_MAN.positions[i].x,
      POLICE_MAN.positions[i].y,
      POLICE_MAN.rotation
    );
    context.lineTo(
      POLICE_MAN.x + (rotatedPoint[0] * POLICE_MAN.scale),
      POLICE_MAN.y + (rotatedPoint[1] * POLICE_MAN.scale)
    );
  }
  context.lineWidth = 1;
  switch (POLICE_MAN.health) {
    case 3:
